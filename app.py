        ...
        if agent_selection == "Escolher um especialista...":
            if agent_selection == "Escolher um especialista...":
                phase_one_prompt = (
                    ...
                    f"4 - \nMETODOLOGIA - 提取详细的工作方法：识别和描述方法：仔细分析工作，以识别所使用的方法。详细描述方法的各个方面，包括研究设计、使用的技术和工具、数据收集和分析程序、方法步骤以及采用的任何特定或创新方法。将描述限制在 250 字以内，以保持简洁而不牺牲重要细节。\n"
                    f"5 - \nDATASET - 识别工作中使用的数据集。简要描述，限制为 40 字。同一行列出数据集名称并用逗号分隔。如果数据集是作者创建的，则写“OWN DATASET”。\n"
                    f"6 - \nRESULTADOS - 用一个段落描述研究结果，强调定量数据，并明确提供数值数据。这个段落还应强调与先前最好的研究相比，新工作的结果。不要使用最高级。保持中立和科学的语气。\n"
                    f"7 - \nLIMITAÇÕES - 转述研究的局限性。\n"
                    f"8 - \nCONCLUSÃO - 总结作者对工作的结论。\n"
                    f"9 - \nFUTURO - 提取对未来研究的建议：根据文章的结论指出对未来研究的建议。\n"
                    f"10 - \nAVALIAÇÃO - 对工作进行批判性评价。不要泛泛而谈，要深入分析。\n"
                    f"</solicitacoes>\n"
                    f"\n\n<controle>\n"
                    f"控制级别：\n"
                    f"1. \n语气：正式科学。\n"
                    f"2. \n主题焦点：您应始终高度关注科学文章的文本。\n"
                    f"3. \n语言：始终以巴西葡萄牙语回答，如巴西人通常撰写科学文章一样，遵循该国的科学写作标准，除非另有说明。\n"
                    f"4. \n情感控制：中立和科学。避免使用最高级，如：创新、革命性等。\n"
                    f"5. \n原创性水平：10，其中 1 是低原创性，10 是高原创性。绝不要复制原文中的句子。\n"
                    f"6. \n抽象水平：1，其中 1 是非常具体和真实，10 是非常抽象和不真实。\n"
                    f"7. \n动词时态：使用过去时。\n"
                    f"</controle>\n"
                    f"\n\n<restricoes>\n"
                    f"不应从英语翻译成葡萄牙语的内容：\n"
                    f"1. \n在葡萄牙语文章中广泛接受和使用的英语技术术语。\n"
                    f"2. \n机器学习算法的名称。\n"
                    f"3. \n工作中使用的指标。\n"
                    f"4. \n数据集名称。\n"
                    f"5. \n不要将 YAML 的返回值包含在 ```yaml 中。\n"
                    f"6. \n不要在返回文本中包含 ``` 或 ```。\n"
                    f"</restricoes>\n"
                    f"\n\n<template>\n"
                    f"文章：\n"
